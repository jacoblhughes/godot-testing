...
func _ready():
	navigation_agent.target_reached.connect(_on_garbage_target_reached)

#
func _physics_process(delta):
	var direction = Vector2.ZERO
	print(garbage_locations)
	if not garbage_locations.is_empty() and not garbage_seeking:
...
func _on_garbage_target_reached():
